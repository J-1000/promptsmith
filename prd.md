...
CREATE TABLE chain_steps (
  ...
  input_mapping JSON,             -- {"var": "{{input.x}}" | "{{steps.key.output}}"}
...
### 1. Dashboard (`/`) ✅
- Project overview
- Recent activity feed (timeline with icons per event type)
- Quick stats (prompts, tests, benchmarks)
- Prompt health indicators (colored dots, pass rates)
...
### 11. Chains (`/chains`) ✅
- List all prompt chains with search/filter
- Step count badges, descriptions
- Create new chain modal

### 12. Chain Detail (`/chains/:name`) ✅
- Step editor: prompt dropdown, output key, input mapping fields, reorder/delete
- Visual connectors between steps
- Run panel: auto-detected input fields, model selector
- Expandable per-step results with rendered prompt, output, duration
...
DELETE /api/chains/:name            Delete chain (cascade)
PUT    /api/chains/:name/steps      Bulk replace chain steps
POST   /api/chains/:name/run        Execute chain
GET    /api/chains/:name/runs       List chain run history
```
...
- [x] Basic web UI scaffolding (React + Vite)

### Phase 3: Testing (Weeks 9-12)
- [x] Test suite definition format
- [x] Assertion types implementation
- [x] Test runner (CLI and web)
...
### Phase 5: Cloud Sync & Collaboration (Weeks 17-20)
- [x] Cloud sync functionality
- [x] Team collaboration features
- [x] Authentication (login/logout)

### Phase 6: Full Web UI (Weeks 21-24)
- [x] All web pages (tests, benchmarks, settings, editor)
- [x] Dashboard stats, prompt CRUD

### Phase 7: CRUD Completion + Polish
- [x] Full CRUD for prompts, tags
- [x] Toast/confirm dialogs
- [x] MIT license, contributing guide

### Phase 8: Feature Completion + Docs
- [x] CodeMirror editor, inline comments, snapshot testing
- [x] Benchmark compare, flaky detection, export reports
- [x] VitePress documentation site

### Phase 9: Interactive Playground
- [x] Playground page with LLM execution, variable filling, model selection

### Phase 10: Dashboard Enhancement
- [x] Activity feed (recent versions, test runs, benchmark runs)
- [x] Per-prompt health indicators (pass rate, status dots)

### Phase 11: Prompt Chains / Pipelines
- [x] Chain data model (chains, chain_steps, chain_runs tables)
- [x] Chain CRUD API (list, create, get, update, delete with cascade)
- [x] Chain step management (bulk replace, input mapping resolution)
- [x] Chain execution (resolve mappings → load prompt → render → LLM → store)
- [x] CLI: `chain list|create|show|run` subcommands
- [x] ChainsPage: list with search, cards, create modal
- [x] ChainDetailPage: step editor, run panel, expandable results
